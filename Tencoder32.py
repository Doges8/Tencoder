...
def Encode():
    text = e1.get()
    print(text)
    encoded = base64.b32encode(text.encode('ascii'))
    e1.delete(0, END)
    e1.insert(0, encoded)
    print(encoded)
...
def Decode():
    ...
    print(text)
    decoded = base64.b32decode(text.decode('ascii'))
    e2.delete(0, END)
    e2.insert(0, decoded)
    print(decoded)
...
t = Label(root, text="Tencoder32 encodes text to bytes using base32, for other methods of encoding, please download a different version of Tencoder", fg="green")
